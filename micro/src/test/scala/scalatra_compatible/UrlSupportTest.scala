...
class UrlSupportTest extends ScalatraFunSuite {
  ...
  def callUrlAndParamsToString(url: String, params: Map[String, String] = Map.empty) =
    get("/context/", params + ("url" -> url)) { response.body }

  test("a page-relative URL should not have the context path prepended") {
    callUrlAndParamsToString("page-relative") should equal("page-relative")
  }

  test("a should expand an option") {
    callUrlAndParamsToString("page-relative", Map("id" -> "the-id")) should equal("page-relative?id=the-id")
  }

  test("a context-relative URL should have the context path prepended") {
    callUrlAndParamsToString("/context-relative") should equal("/context/context-relative")
  }

  test("an absolute URL should not have the context path prepended") {
    callUrlAndParamsToString("http://www.example.org/") should equal("http://www.example.org/")
  }

  test("empty params should not generate a query string") {
    callUrlAndParamsToString("foo", Map.empty) should equal("foo")
  }
  ...
  test("params should be rendered as a query string") {
    ...
    val result = callUrlAndParamsToString("en-to-es", params)
  ...
  test("params should url encode both keys and values in UTF-8") {
    callUrlAndParamsToString("de-to-ru", Map("fünf" -> "пять")) should equal("de-to-ru?f%C3%BCnf=%D0%BF%D1%8F%D1%82%D1%8C")
  }

  test("encodes URL through response") {
    session {
      // NOTE: skinny-micro never appends jsessionid
      callUrlAndParamsToString("foo", Map("session" -> "session")) should not include ("jsessionid=")
    }
  }
}

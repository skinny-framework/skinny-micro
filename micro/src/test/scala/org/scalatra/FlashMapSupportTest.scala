...
class FlashMapSupportSecondTestServlet extends SkinnyMicroServlet with FlashMapSupport {
  post("/redirect") {
...
class FlashMapSupportTestFilter extends SkinnyMicroFilter with FlashMapSupport {
  get("/filter") {
    flash.get("message") foreach { x => response.setHeader("message", x.toString) }
  }

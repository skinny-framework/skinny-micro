...
import skinny.micro.{ MultiParams, Params, SkinnyMicroException }

/**
 * Implicit conversion for MicroParams.
 */
trait SkinnyMicroParamsImplicits {
  ...
  sealed class TypedParams(params: Params) {
    ...
    def as[T <: Any](name: String)(implicit tc: TypeConverter[String, T]): T =
      getAs[T](name) getOrElse (throw new SkinnyMicroException("Key %s could not be found.".format(name)))

    def as[T <: Date](nameAndFormat: (String, String)): Date =
      getAs[T](nameAndFormat) getOrElse (throw new SkinnyMicroException("Key %s could not be found.".format(nameAndFormat._1)))
...
object SkinnyMicroParamsImplicits
  extends SkinnyMicroParamsImplicits

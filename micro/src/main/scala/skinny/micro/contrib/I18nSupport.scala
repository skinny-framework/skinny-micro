...
trait I18nSupport { this: SkinnyMicroBase with BeforeAfterDsl =>
  ...
  def locale(implicit ctx: SkinnyContext): Locale = {
    if (request(ctx) == null) {
  ...
  def userLocales(implicit ctx: SkinnyContext): Array[Locale] = {
    if (request(ctx) == null) {
  ...
  def messages(key: String)(implicit ctx: SkinnyContext): String = messages(ctx)(key)

  def messages(implicit ctx: SkinnyContext): Messages = {
    if (request(ctx) == null) {
   ...
   */
  def provideMessages(locale: Locale)(implicit ctx: SkinnyContext): Messages = Messages(locale)
  ...
  */
   ...
   */
  private def resolveHttpLocaleFromUserAgent(implicit ctx: SkinnyContext): Option[Locale] = {
   ...
   */
  private def localeFromString(in: String)(implicit ctx: SkinnyContext): Locale = {
